...
function DistanceMarker(props) {
	...
	return (
		<Fragment>
			...
			>{`${props.d}m`}</text>
			<line
				class="distanceMarkerLine"
				x1={`${props.x}%`}
				y1={`${y}%`}
				x2={`${props.x}%`}
				y2={`${y + (props.up ? -2.5 : 2.5)}%`}
...
export function RaceTrack(props) {
	...
	const xOffset = props.xOffset || 0,
		...
		xExtra = props.xExtra || 0,
		yExtra = props.yExtra || 0;
	...
	function doMouseMove(e) {
		...
		text.setAttribute(
			'x',
			innerX > props.width - 45 ? innerX - 45 : innerX + 5,
		);
		text.setAttribute('y', innerY);
		text.textContent =
			Math.round(fracX * courses[svg.dataset.courseid].distance) + 'm';
		...
		if (draggedSkill) {
			...
			const newStart = Math.round(
				Math.max(
					0,
					Math.min(course.distance, (x / w) * course.distance - dragOffset.x),
				),
			);
			const skillLength = Math.max(
				50,
				draggedSkill.originalEnd - draggedSkill.originalStart,
			); // Ensure minimum length of 50m
			const newEnd = Math.round(
				Math.max(
					newStart + skillLength,
					Math.min(course.distance, newStart + skillLength),
				),
			);
	...
	const almostEverything = useMemo(() => {
		const flatLevel = 50;
		const [_, highestPoint, lowestPoint] = course.slopes.reduce(
			(x, s) => {
			...
			if (s.slope == 0) {
				elems.push(
					<rect
						...
						width={`${(s.length / course.distance) * 100}%`}
						height="26.2%"
						fill="rgb(211,243,68)"
					/>,
				);
				...
				elems.push(
					<svg
						class={`hillArea ${s.slope < 0 ? 'downhill' : 'uphill'}`}
						x={`${(s.start / course.distance) * 100}%`}
						y="0"
						width={`${(s.length / course.distance) * 100}%`}
						height="26.2%"
		...
		return (
			<Fragment>
				...
				{course.slopes.map((s) => (
					<svg
						class={`slope ${s.slope > 0 ? 'uphill' : 'downhill'}`}
						x={`${(s.start / course.distance) * 100}%`}
						y="28%"
						width={`${(s.length / course.distance) * 100}%`}
						height="18%"
					>
						...
						/>
						<SectionText
							id={s.slope > 0 ? 'uphill' : 'downhill'}
							w={s.length / course.distance}
						/>
					</svg>
				))}
				{course.slopes.map((s, i) => {
					...
					if (
						...
						(i == 0 ||
							s.start !=
								course.slopes[i - 1].start + course.slopes[i - 1].length)
					) {
						markedStart = true;
						nodes.push(
							<DistanceMarker
					...
					if (s.start + s.length != course.distance) {
						nodes.push(
							<DistanceMarker
								d={s.start + s.length}
								x={((s.start + s.length) / course.distance) * 100}
								y={42}
								up={markedStart && s.length < course.distance * 0.05}
							/>,
						);
					}
				...
				{course.corners.map((c, i) => (
					...
					>
						...
						<SectionText
							...
							fields={{ n: 4 - ((course.corners.length - i - 1) % 4) }}
						/>
					</svg>
				))}
				{sections.map((s, i) => {
					...
					if (s.start != 0 && (i == 0 || s.start != sections[i - 1].end)) {
						...
						nodes.push(
							<DistanceMarker
								...
								up={
									i > 0 &&
									s.start - sections[i - 1].end < course.distance * 0.05
								}
							/>,
						);
					}
					if (s.end != course.distance) {
						nodes.push(
							<DistanceMarker
								d={s.end}
								x={(s.end / course.distance) * 100}
								y={60}
								up={markedStart && s.end - s.start < course.distance * 0.05}
							/>,
						);
					}
					return <Fragment>{nodes}</Fragment>;
				...
				if (
					...
					const rects = desc.regions.map((r) => {
						...
						if (desc.skillId && desc.umaIndex !== undefined) {
							...
							if (
								horseState &&
								horseState.forcedSkillPositions.has(desc.skillId)
							) {
								const forcedPos = horseState.forcedSkillPositions.get(
									desc.skillId,
								);
								start = forcedPos;
	...
	const statThresholds = course.courseSetStatus
		.map((s) => statStrings[s])
		.join(joiner);
	...
	return (
		<IntlProvider definition={lang == 'ja' ? STRINGS_ja : STRINGS_en}>
			<>
				{trackNameHeader}
				{props.controls && (
					<div class="racetrackControlsSlot">{props.controls}</div>
				)}
				<div
					class="racetrackWrapper"
					style={{
						'--track-width': `${totalWidth}px`,
						'--track-height': `${totalHeight}px`,
					}}
				>
					<svg
						version="1.1"
						...
						onMouseUp={() => setDraggedSkill(null)}
					>
						<svg
							x={props.xOffset}
							y={props.yOffset}
						...
						>
							...
							{regions}
							{props.posKeepLabels &&
								props.posKeepLabels.map((label, index) => (
