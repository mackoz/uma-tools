...
test('roundBlockSeed: distinct, deterministic seeds per round', () => {
...
describe('SkillAccumulator: incremental mean/variance, concatenation, inert/proc tracking', () => {
	test('incremental mean/variance and concatenation', () => {
	...
	test('a skill that never activates is exactly inert', () => {
	...
	test('lengths() returns a fresh array each call, safe to sort in place', () => {
...
describe('evaluateRound: the actual screening decision', () => {
	test('a clearly-good skill should survive and be protected', () => {
	...
	test('a clearly-bad skill should be eliminated once trusted', () => {
	...
	test('a skill below the CI-elimination floor is not eliminated on CI grounds alone', () => {
	...
	test('high-variance, zero-mean skill is not eliminated on a small pool', () => {
	...
	test('exact zero effect and zero procs is inert, regardless of pool composition', () => {
	...
	test('a very tight interval converges and freezes as final even on a non-final round', () => {
	...
	test('budget: keeps exactly `cap` survivors by upper bound, screens the rest as budget', () => {
		...
		for (let i = pool.length - round.cap; i < pool.length; ++i) {
			assert.ok(
				keptIds.has(`c${i}`),
				`expected c${i} to survive the budget cut`,
			);
		}
	});

	test("on the ladder's last round, a surviving skill becomes 'final', not 'refining'", () => {
...
describe("derivePreset: the Pruning slider's derivation", () => {
	test('PRUNING_DEFAULT must reproduce the base preset exactly', () => {
	...
	test("sample depth and round 1's unbounded cap never move at any slider position", () => {
	...
	test('monotonicity across the full sweep, for every preset', () => {
	...
	test('structural invariants hold at every slider position', () => {
	...
	test('the aggressive end of the slider stops sampling strictly more skills this round', () => {
...
test("estimateWorstCaseScenarios: cost model behind the Skill Chart's runtime estimate", () => {
	const base = CHART_LADDERS.balanced;
