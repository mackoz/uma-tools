...
export function validateAndParseUmaJson(json: any): UmaState | null {
	...
	if (
		json.forcedSkillPositions &&
		typeof json.forcedSkillPositions === 'object'
	) {
...
function getRawSlots(): Record<
	string,
	{ data: any; savedAt: number; memo?: string }
> {
	try {
...
export function getSavedSlotNames(): string[] {
	...
	return Object.keys(slots).sort(
		(a, b) => (slots[b].savedAt || 0) - (slots[a].savedAt || 0),
	);
}

export function saveHorseSlot(
	name: string,
	horse: UmaState,
	memo?: string,
): boolean {
...
export function downloadHorseJson(horse: UmaState): void {
	const blob = new Blob([JSON.stringify(horse, null, 2)], {
