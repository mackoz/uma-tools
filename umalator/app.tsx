...
const unreleasedSkillIds = new Set<string>(unreleased.skills);
...
// Skill Chart rarity filter row (replaces the old binary Hide Inherited Uniques toggle -- UI-22).
// Single-select, modeled on gametora.com/umamusume/skills' Rarity row. 'unique' here means
// *inherited* uniques (id[0] === '9') -- a character's own uniques never enter the Chart mode
// candidate pool in the first place (isGeneralSkill excludes rarity >= 3; see baseSkillsToTest
// above), so this is the only "unique" filter that does anything in this mode. 'pink' (Evolved)
// is appended only when the loaded dataset actually has rarity-6 skills -- Global has none today.
const CHART_RARITY_FILTERS: ReadonlyArray<{
	key: string;
	label: string;
	tooltip?: string;
...
}> = [
	{
		key: 'all',
		label: 'All',
		style: { '--seg-bg': 'var(--card)', '--seg-accent': 'var(--accent)' },
	},
	{
		key: 'white',
		label: 'Normal',
		style: {
			'--seg-bg': 'var(--skill-white-bg)',
			'--seg-accent': 'var(--skill-white-accent)',
		},
	},
	{
		key: 'gold',
		label: 'Rare',
		style: {
			'--seg-bg': 'var(--skill-gold-bg)',
			'--seg-accent': 'var(--skill-gold-accent)',
		},
	},
	{
		key: 'inherit',
		label: 'Unique',
		tooltip:
			"Unique — inherited uniques only. A character's own uniques are evaluated in the Uma Chart tab.",
		style: {
			'--seg-bg': 'var(--skill-unique-bg)',
			'--seg-accent': 'var(--skill-unique-accent)',
		},
	},
	...(hasEvolvedSkills
		? [
				{
					...
					label: 'Evolved',
					style: {
						'--seg-bg': 'var(--skill-pink-bg)',
						'--seg-accent': 'var(--skill-pink-accent)',
					},
				},
			]
		: []),
...
function App(props) {
	...
	// Narrows the Chart-mode candidate pool to one rarity tier (see CHART_RARITY_FILTERS above).
	// 'all' is the no-op/default. A dedicated localStorage key, matching skill-analysis-mode/-preset
	// and chartIconFilter's own convention: a candidate-pool work-scoping knob, not part of the race
	// definition, so it doesn't ride in the umalator-settings share-link blob. Falls back to 'all'
	// for a persisted key this build doesn't recognize (e.g. a stale 'pink' selection loaded on a
	// build with no evolved skills).
	const [chartRarity, setChartRarity] = useState<string>(() => {
		const saved = localStorage.getItem('chartRarityFilter');
		return saved && CHART_RARITY_FILTERS.some((f) => f.key === saved)
			? saved
			: 'all';
	});

	useEffect(() => {
		localStorage.setItem('chartRarityFilter', chartRarity);
	}, [chartRarity]);
	const [lastRunChartRarity, setLastRunChartRarity] = useState('all');

	// Global-only: shows umas/uniques datamined from the Global client but not released there yet
	// (see unreleased.json, generated by scripts/add-staged-global-umas.mjs). Off by default so the
	// picker matches what's actually playable; a roster-visibility knob like chartRarity above, so
	// it also stays out of the umalator-settings share-link blob.
	const [showUnreleasedUmas, setShowUnreleasedUmas] = useState<boolean>(
		() => localStorage.getItem('showUnreleasedUmas') === 'true',
	);
	...
	function getUniqueSkills() {
		return Object.keys(skilldata).filter((id) => {
			...
			if (skill.rarity < 4 || !id.startsWith('1')) return false;
	...
	if (mode == Mode.Compare) {
		...
		// Single-select, so unlike iconTypesDirty (a subset check over a Set) this is just: unchanged,
		// or narrowing off a prior 'all' run, is a pure client-side hide (not dirty); anything else
		// (switching rarities, or widening back to 'all') needs a re-run.
		const rarityDirty =
			mode == Mode.Chart &&
							...
							{mode == Mode.Chart && (
								<div id="chartIconFilter">
									<div class="chartFilterRow">
										...
										<Tabs
											variant="segmented"
											selected={chartRarity}
											onSelect={setChartRarity}
											items={CHART_RARITY_FILTERS.map((f) => ({
												key: f.key,
												label: f.label,
												tooltip: f.tooltip,
												style: f.style,
									...
									<div class="chartFilterRow">
										<span class="chartFilterLabel">Type:</span>
