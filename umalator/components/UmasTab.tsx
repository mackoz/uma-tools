...
import { IntlContext, Text } from 'preact-i18n';
...
export const skillGroups = Object.keys(skilldata)
	...
	.sort(
		(a, b) =>
			...
			(skilldata as Record<string, { rarity: number }>)[a].rarity -
				(skilldata as Record<string, { rarity: number }>)[b].rarity ||
			+b - +a,
	)
	.reduce((groups, id) => {
		const groupId = (skillmeta as Record<string, { groupId: string }>)[id]
			.groupId;
...
function costForId(id: number, owned: Map<string, number>): number {
	const meta = skillmeta as Record<
		string,
		{ groupId: string; baseCost: number }
	>;
	...
	return allUmas.filter((uma) => {
		if (f.name) {
			...
			if (
				...
				!outfitName.toLowerCase().includes(q)
			)
				return false;
		}
...
function buildSkillIndex(
	allUmas: DecodedUma[],
	intlSkillNames: Record<string, string>,
): Map<number, string> {
	...
	return (
		<div class="umasAptCell" title={`${label}: ${letter}`}>
			...
			{value > 0 ? (
				<img
					class="umasAptIcon"
					src={`/uma-tools/icons/utx_ico_statusrank_${String(100 + idx).slice(1)}.png`}
					alt={letter}
				/>
			) : (
	...
	return (
		<label class="umasAptFilterCell">
			...
			<select
				...
				value={value}
				onChange={(e) =>
					onChange(aptKey, +(e.target as HTMLSelectElement).value)
				}
			>
	...
	const matches =
		q.length >= 1
			? Array.from(availableSkills.entries())
					.filter(
						([id, name]) =>
							!selected.includes(id) && name.toLowerCase().includes(q),
					)
					.sort((a, b) => {
				...
				{selected.map((id) => {
					...
					return (
						<span key={id} class={`umasSkillChip ${rarityClass}`}>
							{hasMeta && (
								<img src={getSkillIconSrc(idStr)} class="umasSkillChipIcon" />
							)}
							<span class="umasSkillChipName">{name}</span>
							<button
								class="umasSkillChipRemove"
								onClick={() => remove(id)}
								type="button"
							>
								×
							...
							{matches.map(([id, name]) => {
								...
								return (
									...
									>
										{hasMeta && (
											<img
												src={getSkillIconSrc(idStr)}
												class="umasSkillChipIcon"
												loading="lazy"
											/>
										)}
										<span>{name}</span>
			...
			<div class="umasAptFilter">
				<div class="umasAptFilterRow">
					...
					<AptFilterSelect
						aptKey="apt_turf"
						label="Turf"
						value={aptVal('apt_turf')}
						onChange={setApt}
					/>
					<AptFilterSelect
						aptKey="apt_dirt"
						label="Dirt"
						value={aptVal('apt_dirt')}
						onChange={setApt}
					/>
				</div>
				<div class="umasAptFilterRow">
					...
					<AptFilterSelect
						aptKey="apt_short"
						label="Short"
						value={aptVal('apt_short')}
						onChange={setApt}
					/>
					<AptFilterSelect
						aptKey="apt_mile"
						label="Mile"
						value={aptVal('apt_mile')}
						onChange={setApt}
					/>
					<AptFilterSelect
						aptKey="apt_middle"
						label="Middle"
						value={aptVal('apt_middle')}
						onChange={setApt}
					/>
					<AptFilterSelect
						aptKey="apt_long"
						label="Long"
						value={aptVal('apt_long')}
						onChange={setApt}
					/>
				</div>
				<div class="umasAptFilterRow">
					...
					<AptFilterSelect
						aptKey="apt_nige"
						label="Front"
						value={aptVal('apt_nige')}
						onChange={setApt}
					/>
					<AptFilterSelect
						aptKey="apt_senko"
						label="Pace"
						value={aptVal('apt_senko')}
						onChange={setApt}
					/>
					<AptFilterSelect
						aptKey="apt_sashi"
						label="Late"
						value={aptVal('apt_sashi')}
						onChange={setApt}
					/>
					<AptFilterSelect
						aptKey="apt_oikomi"
						label="End"
						value={aptVal('apt_oikomi')}
						onChange={setApt}
					/>
				</div>
			</div>
...
function SortControl({
	...
	return (
		<div class="umasSortControl">
			...
			<select
				class="umasSortSelect"
				value={(sort ?? DEFAULT_SORT).key}
				...
				{known.map((s) => {
					...
					return (
						<div key={idStr} class={`umasSkillPill ${rarityClass}`}>
							<img
								class="umasSkillPillIcon"
								src={getSkillIconSrc(idStr)}
								loading="lazy"
							/>
							<span class="umasSkillPillName">
								<Text id={`skillnames.${idStr}`} />
							</span>
						</div>
					);
				})}
	...
	return (
		<div class="umaCard">
			<div class="umaCardHeader">
				...
				<div class="umaCardTitle">
					...
					{(uma.rank_score != null || uma.create_time) && (
						<div class="umaCardMeta">
							{uma.rank_score != null && (
								<span class="umaCardMetaItem">
									Rating {uma.rank_score.toLocaleString()}
								</span>
							)}
							{uma.create_time && (
								<span class="umaCardMetaItem">{uma.create_time}</span>
							)}
						</div>
					)}
				</div>
				{(actions.onLoadUma1 || actions.onLoadUma2 || actions.onExport) && (
					<div class="umaCardActions">
						{actions.onLoadUma1 && (
							<button
								class="umaCardActionBtn"
								onClick={() => actions.onLoadUma1!(uma)}
								type="button"
						...
						{actions.onLoadUma2 && (
							<button
								class="umaCardActionBtn"
								onClick={() => actions.onLoadUma2!(uma)}
								type="button"
						...
						{actions.onExport && (
							<button
								class={`umaCardActionBtn umaCardActionBtn--export${copied ? ' umaCardActionBtn--copied' : ''}`}
			...
			<div class="umaCardStats">
				...
				).map(([key, label, catIcon]) => (
					<div class="umaCardStat" key={key}>
						<img class="umaStatCatIcon" src={catIcon} alt={label} />
						<img
							class="umaStatRankIcon"
							src={`/uma-tools/icons/statusrank/ui_statusrank_${statRankStr(uma[key])}.png`}
							alt=""
						/>
...
export function UmasTab({
	...
	const intlSkillNames: Record<string, string> =
		intlCtx?.intl?.dictionary?.skillnames ?? {};

	useEffect(() => {
		loadStoredUmas().then((data) => {
			if (data.length > 0) setImportedUmas(data);
		});
	}, []);
	...
	const handleImport = useCallback(async () => {
		...
		try {
			...
			if (decoded.length === 0) {
				setImportError(
					'Could not decode — please check the URL or code and try again.',
				);
			} else {
				setImportedUmas(decoded);
				setInputValue('');
				saveRoster(decoded).then((encoded) =>
	...
	const availableSkills = useMemo(
		() => buildSkillIndex(importedUmas, intlSkillNames),
		[importedUmas, intlSkillNames],
	);
			...
			<div class="umasImportBar">
				<div class="umasImportInputRow">
					...
					{importedUmas.length > 0 && (
						<button
							class="umasClearAllBtn"
							onClick={handleClear}
							title="Remove all imported umas"
						>
							Clear
						</button>
					)}
				...
				<p class="umasImportHint">
					...
					{importedUmas.length > 0 && (
						<span class="umasLoadedCount">
							{' '}
							{importedUmas.length} uma{importedUmas.length !== 1 ? 's' : ''}{' '}
							loaded.
						</span>
					)}
				</p>
				...
				<>
					{showCount && (
						<p class="umasResultCount">
							Showing <strong>{visible.length}</strong> of {importedUmas.length}{' '}
							umas
						</p>
					)}
					{visible.length > 0 ? (
						<div class="umasGrid">
							{visible.map((uma, i) => (
								<UmaCard
									key={`${uma.card_id}-${i}`}
									uma={uma}
									actions={{ onLoadUma1, onLoadUma2, onExport }}
								/>
							))}
						</div>
					) : (
