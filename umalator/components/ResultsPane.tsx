import { Clock, Flag, Heart, Swords, TrendingUp, Zap } from 'lucide-preact';
...
interface AllRunsData {
	...
	skBasinn: [
		Map<string, [number, number][]> | Record<string, [number, number][]>,
		Map<string, [number, number][]> | Record<string, [number, number][]>,
	];
	totalRuns: number;
...
interface RaceSnapshot {
	...
	// HP-7 pt.2: incoming stamina-debuff proc positions, alongside sk above -- see compare.ts's
	// debuffPos1/debuffPos2. Same [pos, -1] "no end" shape as sk so the two can share
	// skillEntries/skillSize and the row markup below.
	db: [
		Map<string, number[][]> | Record<string, number[][]>,
		Map<string, number[][]> | Record<string, number[][]>,
	];
...
function skillEntries(
	sk: Map<string, number[][]> | Record<string, number[][]>,
): [string, number[][]][] {
		...
		<div class="results-summary">
			{entries.map(({ key, label }) => (
				...
				>
					...
					<span
						class={`results-stat-value ${stats[key] <= 0 ? 'uma1' : 'uma2'}`}
					>
...
function UmaStatsCard({
	...
	const skillNameDict: Record<string, string> =
		(useContext(IntlContext as any) as any)?.intl?.dictionary?.skillnames ?? {};
			...
			<div class="uma-stats-details">
				<div class="stats-primary">
					...
					<div class="stat-item">
						<Heart size={14} />
						<span class="value">
							{staminaStats.staminaSurvivalRate.toFixed(1)}%
						</span>
						...
						<div class="stat-row">
							...
							<span class="value">
								...
								<span class="stat-subtext">
									{' '}
									(
									{staminaStats.hpDiedPositionStatsNonFullSpurt.min?.toFixed(0)}{' '}
									–{' '}
									{staminaStats.hpDiedPositionStatsNonFullSpurt.max?.toFixed(0)}
									,{' '}
									{(
										(staminaStats.hpDiedPositionStatsNonFullSpurt.count /
											(allruns.totalRuns || 1)) *
										100
									).toFixed(1)}
									% of runs)
					...
					{staminaStats.hpDiedPositionStatsFullSpurt.count > 0 && (
						<div class="stat-row">
							...
							<span class="value">
								...
								<span class="stat-subtext">
									{' '}
									({staminaStats.hpDiedPositionStatsFullSpurt.min?.toFixed(0)} –{' '}
									{staminaStats.hpDiedPositionStatsFullSpurt.max?.toFixed(0)},{' '}
									{(
										(staminaStats.hpDiedPositionStatsFullSpurt.count /
											(allruns.totalRuns || 1)) *
										100
									).toFixed(1)}
									% of runs)
					...
					{staminaStats.nonFullSpurtVelocityStats.count > 0 && (
						<div class="stat-row">
							...
							<span class="value">
								...
								<span class="stat-subtext">
									{' '}
									({staminaStats.nonFullSpurtVelocityStats.min?.toFixed(2)} –{' '}
									{staminaStats.nonFullSpurtVelocityStats.max?.toFixed(2)})
								</span>
							</span>
						</div>
					)}
					{staminaStats.nonFullSpurtDelayStats.count > 0 && (
						<div class="stat-row">
							...
							<span class="value">
								...
								<span class="stat-subtext">
									{' '}
									({staminaStats.nonFullSpurtDelayStats.min?.toFixed(2)} –{' '}
									{staminaStats.nonFullSpurtDelayStats.max?.toFixed(2)})
								</span>
							</span>
						</div>
					)}
							...
							{skillEntries(skillMap).map(([skillId, activations]) =>
								activations.map((pos, i) => (
									<div key={`${skillId}-${i}`} class="skill-activation">
										<span
											class="skill-name"
											title={getSkillName(skillId, skillNameDict)}
										>
					...
					<div class="debuffs-section">
						...
						<div class="skill-activations">
							{skillEntries(debuffMap).map(([skillId, activations]) => {
								...
								return activations.map((pos, i) => (
									<div
										key={`${skillId}-${i}`}
										class="skill-activation debuff-activation"
									>
										<span
											class="skill-name"
											title={getSkillName(skillId, skillNameDict)}
