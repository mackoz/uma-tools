...
import { getStoredApiKey, storeApiKey, type UmaState } from '../storage';
...
interface OCRModalProps {
	...
	// UI-35: coarse failure telemetry -- no raw error text, just which of the two
	// client-observable failure modes happened (see handleExtract below). Optional/no-op so this
	// stays usable without a caller that cares about it.
	onError?: (reason: 'no-key' | 'api-error') => void;
}
...
export function OCRModal({
	...
	async function handleExtract() {
		...
		try {
			...
			const result = await extractHorseDataFromImage(
				base64,
				mimeType,
				apiKey.trim(),
			);
		...
		>
			<div class="ocrModal">
				<div class="ocrModalHeader">
					...
					<button class="ocrModalClose" onClick={onClose}>
						✕
					</button>
				</div>
				...
				{step === 'upload' ? (
					<div class="ocrModalBody">
						...
						<div class="ocrInfoNote">
							<strong>Tip:</strong> If you play on PC, you can import all your
							umas at once from the <strong>Umas tab</strong>!
						</div>

						{error && <div class="ocrError">{error}</div>}

						<div class="ocrModalFooter">
							<button class="ocrBtnSecondary" onClick={onClose}>
								Cancel
							</button>
							<button
								class="ocrBtnPrimary"
								...
								<div class="ocrReviewGrid">
									...
									<div class="ocrReviewItem ocrReviewStats">
										...
										<span class="ocrReviewValue">
											SPD {ocrData?.speed} / STA {ocrData?.stamina} / PWR{' '}
											{ocrData?.power} / GUT {ocrData?.guts} / WIS{' '}
											{ocrData?.wisdom}
										</span>
									</div>
