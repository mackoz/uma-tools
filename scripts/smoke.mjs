...
const artifactsDir = path.resolve(root, argValue('--artifacts', 'scripts/smoke-artifacts'));
...
try {
	...
	console.log('smoke SKIPPED: playwright not installed (npm i -D playwright && npx playwright install chromium)');
	process.exit(2);
}
...
async function check(id, fn, meta = {}) {
	...
	try {
		...
		rec.ok = false;
		rec.message = String(e.message || e).split('\n').slice(0, 3).join(' ');
		if (currentPage) {
			const shot = path.join(artifactsDir, `fail-${id.replace(/[^\w.-]/g, '_')}-${currentTheme}.png`);
			try {
...
async function assertContrast(page, id, selector, minOverride) {
	await check(id, async () => {
		const c = await page.evaluate(sel => window.__smoke.contrast(sel), selector);
		if (!c || c.error) fail(`${selector}: ${c ? c.error : 'no result'}`);
		const min = minOverride ?? (c.large ? 3.0 : 4.5);
		if (c.ratio < min) {
			fail(`${selector}: contrast ${c.ratio} < ${min} (fg ${c.fg} on bg ${c.bg})`);
		}
		return { selector, expected: `>= ${min}`, actual: c.ratio, fg: c.fg, bg: c.bg };
	}, { selector });
}
...
async function runTheme(browser, url, theme) {
	...
	await context.route('**/*', route => {
		...
		if (host === 'localhost' || host === '127.0.0.1' || host === '[::1]') route.continue();
		else route.abort();
	});
	const page = await context.newPage();
	currentPage = page;
	page.setDefaultTimeout(5000);
	page.on('pageerror', e => pageErrors.push({ theme, message: String(e.message || e) }));
	page.on('console', msg => {
	...
	await check('theme.init', async () => {
		...
		if (isDark !== (theme === 'dark')) {
			fail(`html.dark is ${isDark}, expected ${theme === 'dark'} for theme=${theme}`);
		}
	...
	await check('clip.navbar', async () => {
		...
		if (r.x < 0 || r.right > 1280) fail(`#navBar overflows horizontally (x ${r.x}, right ${r.right})`);
		return { selector: '#navBar', actual: `x ${Math.round(r.x)}..${Math.round(r.right)}` };
	});

	// contrast on always-visible chrome
	await assertContrast(page, 'contrast.modetab-active', '#modeTabs .tabsItem.active');
	await assertContrast(page, 'contrast.modetab-inactive', '#modeTabs .tabsItem:not(.active)');
	await assertContrast(page, 'contrast.uma-search-input', 'input.umaSelectInput');
	...
	await check('search.uma', async () => {
		...
		await page.waitForSelector('ul.umaSuggestions.open li.umaSuggestion', { state: 'visible' });
		const n = await page.locator('ul.umaSuggestions.open li.umaSuggestion').count();
		await page.keyboard.press('Escape');
		if (n < 1) fail('no uma suggestions for "spe"');
		return { selector: 'ul.umaSuggestions.open li.umaSuggestion', expected: '>= 1', actual: n };
	});
	...
	if (pickerOpened) {
		await check('zindex.picker-above-sidebar', async () => {
			...
			if (!(z.picker > z.sidebar)) {
				fail(`.skill-picker-overlay z ${z.picker} not above #iconSidebar z ${z.sidebar}`);
			}
		...
		await check('modal.picker-filter', async () => {
			...
			if (after >= before && before > 20) fail(`filter had no effect (${before} -> ${after})`);
			return { selector: '.skill-picker-item', expected: `1..${before - 1}`, actual: after };
		});

		await check('clip.picker-modal', async () => {
			const r = await page.evaluate(sel => window.__smoke.rect(sel), '.skill-picker-modal');
			if (!r) fail('.skill-picker-modal not found');
			if (r.x < 0 || r.right > 1280 || r.y < 0 || r.bottom > 800) {
				fail(`modal overflows viewport (x ${Math.round(r.x)}..${Math.round(r.right)}, y ${Math.round(r.y)}..${Math.round(r.bottom)})`);
			}
			...
			if (list.overflowY !== 'auto' && list.overflowY !== 'scroll') {
				fail(`.skill-picker-list overflow-y is ${list.overflowY}, long lists would clip`);
			}
			return { selector: '.skill-picker-modal' };
		});

		await assertContrast(page, 'contrast.picker-item', '.skill-picker-item .skill-picker-item-name, .skill-picker-item');
		await assertContrast(page, 'contrast.picker-search', '.skill-picker-search input');

		await shoot(page, 'skillpicker');

		await check('modal.picker-close', async () => {
			await page.locator('.skill-picker-close').click();
			await page.waitForSelector('.skill-picker-overlay', { state: 'detached' });
	...
	const infoOpened = await check('modal.info-open', async () => {
		await page.locator('#iconSidebar .tabsItem[aria-label="About & changelog"]').click();
	...
	if (infoOpened) {
		await check('zindex.infomodal-below-sidebar', async () => {
			...
			if (!(z.overlay < z.sidebar)) {
				fail(`.infoModalOverlay z ${z.overlay} should stay below #iconSidebar z ${z.sidebar} (intentional, see UI-7)`);
			}
	...
	const chartOpened = await check('chart.tab', async () => {
		await page.locator('#modeTabs .tabsItem', { hasText: 'Skill Chart' }).click();
		await page.waitForSelector('#chartRunSettings', { state: 'visible' });
		const active = await page.locator('#modeTabs .tabsItem.active').textContent();
		if (!/Skill Chart/.test(active || '')) fail(`active mode tab is "${active}", not Skill Chart`);
	...
	if (chartOpened) {
		await check('chart.controls', async () => {
			for (const sel of ['#chartIconFilter', '#run']) {
				if (!(await page.locator(sel).first().isVisible())) fail(`${sel} not visible on Skill Chart tab`);
			}
		...
		if (chartRun) {
			await check('chart.run', async () => {
				...
				await page.waitForSelector('table.basinnChart tbody tr', { state: 'visible', timeout: 120_000 });
				const rows = await page.locator('table.basinnChart tbody tr').count();
				return { selector: 'table.basinnChart tbody tr', expected: '>= 1', actual: rows };
			});
		}
	...
	await check('search.umastab', async () => {
		...
		if (!(await page.locator('.umasEmpty').isVisible())) fail('empty-roster state not shown');
		await input.fill('');
	...
	await check('boot.pageerror', async () => {
		const errs = pageErrors.slice(errorsBefore).filter(e => e.theme === theme);
		if (errs.length) fail(`${errs.length} runtime error(s): ${errs[0].message}`);
...
try {
	...
	if (/executable doesn't exist|browserType.launch/i.test(String(e.message))) {
		console.log('smoke SKIPPED: chromium not installed (npx playwright install chromium)');
		process.exit(2);
	}
	throw e;
}
...
fs.writeFileSync(path.join(artifactsDir, 'smoke-report.json'), JSON.stringify(report, null, '\t') + '\n');

if (failed.length === 0) {
	console.log(`smoke OK (${checks.length} checks, light+dark, ${Math.round(report.durationMs / 1000)}s)`);
	process.exit(0);
} else {
	const head = failed.slice(0, 3).map(c => `${c.id}[${c.theme}] ${c.message || ''}`.trim()).join('; ');
	console.log(`smoke FAILED ${failed.length}/${checks.length}: ${head}`);
	console.log(`detail: ${path.relative(root, path.join(artifactsDir, 'smoke-report.json'))}`);
	process.exit(1);
}
