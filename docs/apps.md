...
- **Build:** `build.mjs` (`node build.mjs [--debug]`), CI-built. A legacy `build.bat` also exists but `build.mjs` is authoritative — it adds the `mockAssert` plugin so `CourseData.ts`'s `node:assert` import resolves in the browser, which the raw `.bat` pipeline doesn't handle: without it, `node:assert` doesn't resolve in the browser and the app throws at module-eval time before rendering anything (this is exactly the failure mode that shipped, uncaught, in `build-planner`'s last committed bundle, before that app was retired in PIPE-84).
