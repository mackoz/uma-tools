...
`add-staged-global-umas.mjs --refresh-staged-meta` (UI-28) closes one narrow, different gap this same limitation used to create: regenerating `umalator-global/skill_meta.json` straight from `master.mdb` (e.g. to add a new field) necessarily drops every staged entry, since `master.mdb` has no idea those umas/skills are being staged early, and the script's normal add-only mode can't restore them (its work list is filtered by `umalator-global/umas.json`, which a `skill_meta.json`-only regeneration never touches, so it finds nothing new to add). This mode instead walks `unreleased.json`'s existing `.skills` list directly and re-copies each id *actually missing* from the just-regenerated `skill_meta.json`, skipping any id the regeneration left in place (that's a sign it's since become genuinely Global-authoritative, not merely still-staged — see the script's own header comment for why this deliberately isn't the same permissive guard `setJpSourced` uses elsewhere). It's a restore-if-missing operation, not the general refresh-if-stale capability the previous paragraph still describes as unbuilt.
