...
**Until that's fixed, if game data just looks stale (missing umas/skills/courses) rather than needing this whole pipeline run, see `scripts/sync-upstream-data.mjs`** — it ports already-computed data from a local upstream checkout instead of extracting from a live client. It's a stopgap for exactly this gap, not a replacement for the pipeline above.
