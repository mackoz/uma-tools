...
`make_uma_info.pl` remains blocked by two things unrelated to the two bugs above: `File::Slurper` isn't installed in every environment this pipeline gets run from (an environment gap, not a code bug), and the encrypted `meta` asset DB it needs. **The next section only unblocks `extract_resource.pl`, not this script** — `make_uma_info.pl:45` derives `meta`'s path from `master.mdb`'s own filename (see `docs/master-mdb-schema.md`'s naming convention: a `master_jp.mdb` argument looks for a sibling `meta_jp`, `master.mdb` for a bare `meta`) but has no way to point it at `decrypt-meta-db.mjs`'s `<input>.decrypted` output instead, so decrypting `meta` doesn't by itself let `make_uma_info.pl` run (PIPE-2 review, 2026-08-25) — someone would need to either overwrite `meta`/`meta_jp` in place with the decrypted copy (defeating `decrypt-meta-db.mjs`'s own "never touch the original" design) or add a CLI override to this script, neither of which has been done. Combined with the still-open `File::Slurper` gap, `make_uma_info.pl` remains fully blocked either way.
...
The section above documents the JP CDN only; `work-queue/completed/pipe-2.md` explicitly flagged the Global equivalent as unverified. PIPE-20 (2026-08-29) confirmed it via a live HAR capture (Reqable) from a real Global client session, pulled off the device with `adb pull`:

- **Domain**: `assets-umamusume-en.akamaized.net` (asset bundles + manifests) and `assets-webview-umamusume-en.akamaized.net` (webview/marketing images) — both Akamai-fronted S3, same architecture as JP's CDN. Re-verified reachable from a plain connection with no VPN and no special headers, unlike JP's documented Japan-network-path requirement above.
- **Path shape differs from JP by one segment**: `/dl/vertical/resources/<Platform>/assetbundles/<hash[0:2]>/<hash>` for asset bundles (same hash-bucketing convention as JP, just with an extra `vertical/` segment), and `/dl/vertical/<appId>/manifests/manifestdat/root.manifest.bsv.lz4?r=<timestamp>` for the root manifest — a **fixed path gated by a numeric app/environment id** (`10007550` observed), not hash-keyed the way JP's manifest rows are. This is why naively probing Global's domain with JP's `/dl/resources/...` shape returns `403 AccessDenied` rather than `404 NoSuchKey` — S3's default response for a missing key when anonymous `ListBucket` isn't granted, easy to misread as "wrong domain."
- **The CDN base URL is server-issued, not a client constant, for either server.** Extracting `global-metadata.dat` from a real Global APK (`assets/bin/Data/Managed/Metadata/global-metadata.dat`) turned up `Gallop.ToolStartSessionResponse`'s `ResourceServerURL`/`ResourceServerUrlForBundle`/`ResourceServerURLForManifest`/`ResourceServerURLForMaster` properties (msgpack keys `resource_server`/`resource_server_cf`/`resource_server_ingame`/`resource_server_login`) — the domain is handed back by the `tool/start_session` API response at login, not baked into the client binary. This is why grepping either client's binary directly for `akamaized` finds nothing, and it explains JP's own CDN domain having migrated three times (`prd-storage-umamusume` → `prd-storage-app-umamusume` → `prd-storage-game-umamusume`, per `katboi01/UmaViewer` commit `3e2a9704`) without ever needing a client update.

**The Global `meta` DB decryption key is a separate, still-unsolved problem.** A real Global `meta` file (pulled the same way, 80,732,160 bytes) has the identical sqleet/ChaCha20 header structure as JP's `meta_jp` (byte-identical page-size/format-version/reserved-space fields — only the salt differs), confirming the cipher scheme matches; only the key differs, as expected per the "JP and Global are independent datasets" section below. JP's `META_DB_HEXKEY` (see `scripts/decrypt-meta-db.mjs`) does not open it. A candidate 33-byte Global key published in a third-party repo (`Apolexian/clairvoyance`'s `extract_story_text.py`, captured via a Frida hook on `sqlite3_key` in the game's `libnative.dll`, dated 2026-07-01) failed against every cipher/PRAGMA combination tried through the same `better-sqlite3-multiple-ciphers` binding this repo's script already uses — most likely because it's rotated since capture, consistent with this game's documented pattern of rotating exactly these kinds of secrets every few months. See PIPE-20 for the full list of attempts and the suggested next step (a live hook via a Hachimi plugin, e.g. `qwcan/CarrotBlender`'s interception approach retargeted at `sqlite3_key` instead of its current `Gallop.HttpHelper.DecompressResponse` hook) if picking this back up.
