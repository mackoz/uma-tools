...
`RaceSolverBuilder` (the `RaceSolverBuilder` class in `uma-skill-tools/RaceSolverBuilder.ts`) is a fluent builder. Callers chain `.course()`, `.horse()`, `.mood()`, `.ground()`, `.addSkill()`, `.pacer()`, etc., then call `.build()` — a **generator** (`RaceSolverBuilder.ts`'s `build()`) that `yield`s one configured `RaceSolver` per Monte Carlo sample. Passing `true` back into `.next()` re-rolls that sample.

`.addOpponentDebuff(skillId)` (HP-7) is the same idea as `.addSkill()` for a debuff landed on this horse *by an opponent* rather than cast by it -- it rewrites the skill's condition down to only an **allowlist** of victim-safe terms (`phase`, `phase_random`, `accumulatetime`, `distance_type`, and the four `running_style_count_{nige,senko,sashi,oikomi}_otherself` terms, which describe the *victim's* own running style once rewritten) via `victimSafeCondition()` before adding it, since a caster-state term (the caster's own stats, position, etc.) can't be evaluated against this horse's own race state. `umalator/compare.ts`'s `addIncomingDebuffs()` is the sole caller, feeding `HorseState.incomingDebuffs`. See `uma-skill-tools/docs/adr/0014-victim-safe-debuff-conditions.md` for the full allowlist rationale.

End-to-end data flow for one `.build()` call:

1. **Stat pipeline.** `buildBaseStats()` (`RaceSolverBuilder.ts:180`) applies overcap halving (any stat above 1200 is halved past that point) and the mood coefficient.
...
3. **Sampling.** `samplePolicy.sample(regions, nsamples, skillRng, spares)` turns each skill's region list into concrete trigger `Region`s per sample — this is *why* `nsamples` exists: skill trigger points are randomized per the game's own activation model, so you run many samples and look at the distribution. For a skill with no cooldown, or a cooldown on a single-point policy (`straight_random`, `is_finalcorner_random`), this is still one ~10m trigger region per sample, exactly as before SKL-21. For a cooldown skill on a multi-point policy (`all_corner_random`, or the distribution/Erlang family), `sample()` also draws `spares` extra candidate regions per sample so the skill can re-arm after its cooldown elapses, returning a fixed `nsamples * (1 + spares)`-length array laid out in a fixed stride — see `ActivationSamplePolicy.ts@successive-trigger-placement` for how successive points are placed, and `RaceSolverBuilder.ts@skl-21-spares-count` for why `spares` is requested only for that family. Note the distribution-family region was never actually a ~10m trigger: it spans from the sampled point to the end of its candidate region — exactly the mechanic the cooldown undershoot explanation (`docs/adr/0019-cooldown-rearm-design.md`) turns on.
4. **Adjusted stats**, deliberately computed *after* sampling (the "must come after skill activations are decided because conditions like base_power depend on base stats" comment in `RaceSolverBuilder.ts`'s `build()`) — course speed modifier, ground modifiers, strategy proficiency on wisdom. Some conditions (e.g. `base_power`) intentionally read pre-adjustment stats.
5. **Per-sample solver construction.** `PendingSkill[]` are built by zipping each skill's `SkillData` with its sampled trigger, plus — for a cooldown skill on a multi-point policy — a `cooldown` value and a `spares` array of that sample's own remaining candidate regions, sliced out of the fixed `nsamples * (1 + spares)` stride laid out in step 3 (`RaceSolverBuilder.ts@skl-21-spares-count`). A non-cooldown skill, or a cooldown skill restricted to a single-point policy, carries `spares: undefined` and re-arms the same way it always did (never). HP tracking is `GameHpPolicy` when `mode === 'compare'`, otherwise `NoopHpPolicy` — the skill/uma chart intentionally ignores stamina.
...
| File | Role |
|---|---|
| `RaceSolver.ts` (1536 lines) | The physics loop. `RaceState`, `SkillType`, `SkillEffect`, `PendingSkill`. Owns ~10 independently-seeded sub-RNGs (`syncRng`, `gorosiRng`, `rushedRng`, `downhillRng[]`, `sectionSpeedRng`, `posKeepRng`, `laneMovementRng`, `specialConditionRng`, `competeFightRng`) so toggling one subsystem doesn't desync the others across a paired comparison run. Wisdom-check rolls don't use a persistent named RNG — each check constructs a fresh `Rule30CARng` seeded from a `skillWisdomSeed` field instead. The same one-shot-RNG-per-roll pattern (HP-6) backs `scaleEffectValue()`'s "Multiply Random" value-scaling roll (`ability_value_usage` 8/9 — 60%/30%/10% → 0.0x/0.02x/0.04x on the effect's modifier): a fresh `Rule30CARng` per activation, seeded from a `skillValueSeed` field keyed by `(skillId, perspective, effectIdx, activationCount)`, for the same desync-avoidance reason as the wisdom checks. Every *deterministic* `ability_value_usage`/`ability_time_usage` code is handled separately, as a pure table lookup in `uma-skill-tools/ValueScaling.ts` against one `ScalingContext` built per effect activation (SKL-7) — value 2/13/22/23 and duration 3/7 are implemented, every other code returns exactly `1.0`. That module is deliberately dependency-free so this repo's Preact skill picker imports the same tables the solver uses instead of mirroring them; `components/ScalingContext.ts` is the UI-side counterpart that builds the context, running a horse through the engine's own `buildBaseStats()`/`buildAdjustedStats()` so the two sides can't feed the same tables different inputs. See `uma-skill-tools/docs/adr/0013-value-scaling-identity-fallthrough.md`. |
