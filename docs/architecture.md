...
`mackoz/uma-skill-tools` is a **heavily modified fork**, not a snapshot — `RaceSolver.ts` has roughly doubled in size relative to `alpha123/uma-skill-tools` (multi-uma field simulation, a 5-state position-keep machine, lane movement, compete/lead competition). Most of that modification isn't this fork's own — it was imported from [`Werseter/uma-skill-tools@kachi`](https://github.com/Werseter/uma-skill-tools/tree/kachi); see `mackoz/uma-tools`'s `plans/engine-comparison/forks.md` for the attribution. See [upstream-comparison.md](upstream-comparison.md) for the full breakdown, including which changes are genuinely unique versus which duplicate a fix upstream made independently in its own engine repo after the fork point.
