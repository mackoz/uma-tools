...
- **Dead `EnhancedHpPolicy` import.** `uma-skill-tools/RaceSolverBuilder.ts:11` still imports `EnhancedHpPolicy` from a file deleted in commit `90356c1` ("Cleanup + add sync RNG checkbox", in this repo's history from before the second submodule conversion); the symbol is never used. `umalator/compare.ts:5` no longer does — it was fixed to `import { PosKeepMode, type RaceSolver } from '../uma-skill-tools/RaceSolver';`. The `RaceSolverBuilder.ts` half is harmless in the browser builds because esbuild elides the unused import, but it contributes to the standalone TypeScript/CLI failures and should be deleted as cleanup.
