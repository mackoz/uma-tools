...
1. **Never hand-edit generated files.** These are all build output or data-pipeline output, not hand-written source:
   ...
   - `umas.json`, `skill_meta.json`, `icons.json`, and the `umalator-global/` equivalents — Perl-generated from the game's `master.mdb`. Edit the generating `.pl` script (see `docs/data-pipeline.md`) and regenerate, don't patch the JSON directly. `uma-skill-tools/data/jp/{skill_data,skillnames,course_data}.json` are the same story but live in the `uma-skill-tools` submodule (see the submodule section below) — the generating `.pl` scripts are there too, and a fix means committing and pushing in that repo, not editing the checked-out copy here. `uma-skill-tools/data/global/{skill_data,skillnames,course_data}.json` are their Global counterparts, generated by parallel `.pl` scripts run against a different (Global) `master.mdb` — `skill_data.json` reuses the exact same script as JP, while `skillnames.json`/`course_data.json` use separate, Global-specific scripts (`make_global_skillnames.pl`/`make_global_course_data.pl`, identical logic to their JP counterparts per `docs/data-pipeline.md`'s comparison table); the engine itself never reads them (`RaceSolverBuilder.ts`/`CourseData.ts` always resolve `data/jp/`) — a build-time redirect plugin sends the Global web app's imports there instead (see `docs/apps.md`). `uma-skill-tools/data/{jp,global}/tracknames.json` and `presets.ts`/`umalator-global/presets.ts` (the CM/LOH preset dropdown data, see `docs/cm-presets.md`) are the exceptions: all are hand-maintained because no generator exists — there's no `make_tracknames.pl`, and future CM cups aren't in any `master.mdb` yet.
...
The `superpowers` plugin's process skills (brainstorming, TDD, systematic-debugging, SDD, etc.) work here as-is, but a few of its defaults collide with this project's 3-repo conventions — the rules below override the skill's own instructions (consistent with superpowers' "user instructions take precedence" rule). Every cross-repo item below was confirmed by a real SDD run (PIPE-42, then HP-6), not hypothesized; the root cause is always the same — SDD assumes a single git repo, and this project structurally never is. Verdict from those runs: worth using again, but budget extra controller effort for the submodule seams and sanity-check its cleanup/landing defaults against this repo's own tooling. The final whole-branch review is **non-optional on cross-repo work**: on HP-6 it caught a user-visible regression no per-task review structurally could (engine tasks moved a scaling factor out of stored data; parent-repo UI renders that value verbatim — the skill picker would have shown "−100.0% HP drain" where "−4.0%" belonged, contradicting the same branch's changelog; no single task's diff contained both halves).
...
- **`finishing-a-development-branch` verifies tests *before* it detects the environment** — a drifted cwd produced a false green (`npm test` passing against `master`, not the feature branch) on PIPE-42. Confirm `git branch --show-current` and `git rev-parse --show-toplevel` yourself before trusting that skill's Step 1 result.
- **Worktree teardown goes through `scripts/stage-for-review.sh`, not `git worktree remove`.** Every worktree in this repo contains the submodule, and `git worktree remove` hard-refuses on submodule-containing worktrees regardless of cleanliness (`deinit -f` doesn't unblock it). The script already handles this (`rm -rf` + `git worktree prune`, after refusing if anything is uncommitted or unpushed) — superpowers' cleanup step doesn't know that refusal exists.
- **Deferred-Minor review findings must carry their full original reasoning to the final review**, not a one-line summary — reclassification is a working feature (a PIPE-42 Minor was correctly upgraded to Important once the final reviewer saw a cross-task interaction invisible in the original task's diff), but only if the final reviewer can re-derive the original judgment.
- **Use absolute paths for every controller-side write** (ledger, `.superpowers/` artifacts, `wq.py` calls) — never rely on a previous `cd` persisting. SDD legitimately requires cd-ing into the submodule (a correct submodule diff needs it), and the drifted cwd then sends the next relative-path write into the wrong tree; this bit three times on HP-6 and is the same failure class as the `finishing-a-development-branch` false green above. A PreToolUse/PostToolUse hook pair (`scripts/hooks/submodule-cwd-guard.mjs`, wired in `.claude/settings.json`, PIPE-49) now enforces the dangerous subset mechanically — relative parent-scoped commands (`npm test`/`npm run`, `plans/scripts/wq.py`, `scripts/*`) are denied outright while cwd is inside the submodule (absolute-path invocations and `npm --prefix` pass, so the sanctioned workarounds stay open), and every other command from there gets a drift warning injected.
- **Generated-data diffs don't get read, they get verified.** A task that regenerates JSON produces MB-scale review packages; instruct that task's reviewers to verify programmatically (extract the base version via `git show <base>:<file>`, then a structural-comparison script) instead of reading hunks linearly — on HP-6 that caught more than a linear read would have, at a fraction of the context.
- **"Continuous execution" means no permission-seeking check-ins, not silence.** Post a one-line status between SDD tasks; a run that goes an hour with nothing surfaced reads as stalled to the user even when it's healthy.
...
- `uma-skill-tools/` is intentionally engine-only — it must stay independent of Preact/DOM so it can run inside the web worker (`simulator.worker.ts`) and the `tsx` CLI tools under `uma-skill-tools/tools/`. Don't import Preact components into it.
