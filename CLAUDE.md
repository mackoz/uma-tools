...
- **`/wq` drives the mechanics of the above** (`.claude/skills/wq/`) — the exact `file`/
  ...
  words, not free text; there's no `--category` flag), plus the `--park` escape hatch
  through a leftover-staged-residue refusal and the rollback `file`/`status` roll back on
  a rejected commit. Doesn't cover `complete` or landing — see `/project-land` for those.
...
- **Worktrees live under `.claude/worktrees/` and need setup**: run `scripts/worktree-setup.sh` inside a fresh worktree (submodule init, `node_modules` + `plans/` symlinks, git-exclude), and `scripts/stage-for-review.sh <branch>` to tear one down and stage its branch in the main checkout. A dev server started from a worktree derives its static root from the repo path — the app serves at `http://localhost:8000/<worktree-name>/umalator-global/` and `/uma-tools/`-prefixed assets (icons, fonts) 404 there. That's expected inside a worktree; do final visual checks from the main checkout.
- **`/project-review` reviews one linked group of open PRs across the three repos** (`.claude/skills/project-review/`) — the review-side counterpart to `wq.py land`'s merge. Discovery finds every open PR per repo, groups them by real evidence (a PR body cross-linking another, or a shared leading ticket ID — a shared branch name alone only corroborates), and reviews the selected group with `/code-review`, engine → code → plans, then a cross-repo pass for things a single-repo review can't see (a gitlink bump that must match a specific engine squash-merge commit, an engine signature change `uma-tools` depends on, a doc claim in `uma-tools-plans` a code change makes stale) whenever two or more repos are represented in that group. A repo with more than one open PR, or several open PRs that aren't actually related, no longer blocks it — the unlinked ones are reported as excluded rather than lumped in or stopped on. A single open PR is still a fully supported case, and still skips the cross-repo pass. Never merges or lands anything itself.
...
- **Every PR merges as a squash commit.** As of 2026-09-02 (PIPE-38) all three repos have merge commits and rebase-merges disabled in their GitHub settings — squash is the only enabled method, and `wq.py land` uses `gh pr merge --squash`. Consequence: a PR's branch-tip commit never lands on the default branch verbatim; what lands is one new single-parent squash commit. Pre-PIPE-38 completed tickets in `plans/work-queue/` that cite `--merge` merges are accurate historical records, not instructions.

## Superpowers plugin interop

The `superpowers` plugin's process skills (brainstorming, TDD, systematic-debugging, SDD, etc.) work here as-is, but a few of its defaults collide with this project's 3-repo conventions — the rules below override the skill's own instructions (consistent with superpowers' "user instructions take precedence" rule). All four cross-repo items were confirmed by a real SDD run on PIPE-42, not hypothesized.

- **Plan documents never land in this repo.** `writing-plans`' default save location (`docs/superpowers/plans/`) is overridden: the plan lives in the work-queue ticket's `## Plan` section (plans repo) and/or the session plan file. Same for SDD's `.superpowers/sdd/` artifacts — fine as local working state, never committed.
- **Worktrees still need `scripts/worktree-setup.sh`** after *any* creation path (`using-git-worktrees`, native EnterWorktree, plain `git worktree add`) — it now also recreates the `plans/` symlink, without which work-queue tickets are unreachable from the worktree. And the Agent tool's `isolation: "worktree"` parameter is **not** "use the worktree I prepared" — it cuts a brand-new, un-set-up worktree off master (submodule empty, no symlinks); don't pass it when dispatching subagents into a worktree that already exists.
- **SDD's workspace is per-git-repo, and the submodule is its own git repo.** Its tooling derives paths from `git rev-parse --show-toplevel`, so running it from inside `uma-skill-tools/` silently splits ledger/artifacts into a second workspace. Run SDD tooling from the uma-tools root consistently, even for engine tasks. Relatedly: a task that intentionally leaves a broken intermediate state across the submodule boundary (e.g. a gitlink bump whose companion fix is the *next* task) needs that recorded as a preflight ruling up front, or per-task reviewers will correctly flag "doesn't build" as Critical.
- **`finishing-a-development-branch`'s "merge back locally" option is never valid here**, and its one-repo/one-branch model can't express how tickets land: as a *set* (engine → code → plans, gitlink re-bump in between) via `/project-land`. "Finish" for ticket work means: every repo's PR open and ready, then hand the whole set to `/project-land` — never merge one repo's PR of a set in isolation.
- **Subagent dispatch on Fable sessions**: SDD/review skills' generic `general-purpose` dispatches are replaced by the `plan-implementer` agent (implementation, stops before push) and the `fable-reviewer` peer session (review). Non-Fable sessions dispatch as the skills describe.
...
npm run test                                 # unit tests: statisticalAnalysis.ts, chartLadder.ts, shopSkillFilter.ts, spOptimizer.ts, racePresets.ts, histogramData.ts, BasinnChart.tsx, via Vitest (`vitest run`); test:stats is a kept alias
