...
- **`/project-review` reviews one linked group of open PRs across the three repos** (`.claude/skills/project-review/`) — the review-side counterpart to `wq.py land`'s merge. Discovery finds every open PR per repo, groups them by real evidence (a PR body cross-linking another, or a shared leading ticket ID — a shared branch name alone only corroborates), and reviews the selected group with `/code-review`, engine → code → plans, then a cross-repo pass for things a single-repo review can't see (a gitlink bump that must match a specific engine merge commit, an engine signature change `uma-tools` depends on, a doc claim in `uma-tools-plans` a code change makes stale) whenever two or more repos are represented in that group. A repo with more than one open PR, or several open PRs that aren't actually related, no longer blocks it — the unlinked ones are reported as excluded rather than lumped in or stopped on. A single open PR is still a fully supported case, and still skips the cross-repo pass. Never merges or lands anything itself.
