...
- **`/paired-review` reviews all three repos' open PRs together** (`.claude/skills/paired-review/`) — the review-side counterpart to `wq.py land`'s paired merge. It runs `/code-review` on each of `uma-skill-tools`/`uma-tools`/`uma-tools-plans`'s open PR in that order, then a cross-repo pass for things a single-repo review can't see (a gitlink bump that must match a specific engine merge commit, an engine signature change `uma-tools` depends on, a doc claim in `uma-tools-plans` a code change makes stale). Never merges or lands anything itself.
...
- **`// ANCHOR: <name>` comments** (this file and `uma-skill-tools/`) are cited by name from the private `uma-tools-plans` repo's docs (PIPE-8) — `RaceSolver.ts@spot-struggle-duration` instead of a line number, so the citation survives the line moving. Don't delete or rename one without checking whether it's cited (`uma-tools-plans/scripts/check-citations.py`, if that sibling repo is checked out) — a marker can move with its code freely, it just can't disappear or be renamed silently.
