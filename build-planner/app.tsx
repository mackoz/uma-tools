...
function App(props) {
	...
	Object.keys(skillnames).forEach((id) => {
		skilldefs.skillnames[id] = skillnames[id][lang];
	});
	...
	return (
		<IntlProvider definition={skilldefs}>
			<RaceTrack
				...
				width="800"
				height="220"
				regions={[regionsForSkill(c11203, '201452')]}
			/>
			<div style="width:1000px;height:50px;overflow:hidden" />
			<RaceTrack courseid="10805" width="800" height="220" />
			<div style="width:1000px;height:50px;overflow:hidden" />
			<RaceTrack courseid="10810" width="800" height="220" />
			<SkillList
				ids={Object.keys(skills).filter(
					(id) => skills[id].rarity < 3 || skills[id].rarity > 5,
				)}
			/>
		</IntlProvider>
	);
